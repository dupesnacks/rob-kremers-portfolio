...
export default function App() {
 ...
 const [oRes, tRes, bRes, vRes, pRes, sRes, schRes, aRes] = await Promise.all([
 api(`/api/outfits${wq}`),
 api(`/api/tickers${wq}`),
 api(`/api/bias${wq}`),
 api("/api/vix"),
 ...
 *::-webkit-scrollbar-thumb { background: #333; border-radius: 3px; }
 ...
 <span style={{
 fontSize: "10px",
 color: schwabStatus?.authenticated ? GREEN : YELLOW,
 border: `1px solid ${schwabStatus?.authenticated ? "#006600" : "#665500"}`,
 padding: "2px 8px", borderRadius: "3px",
 }}>
 {schwabStatus?.authenticated ? "● SCHWAB" : schwabStatus ? "○ YFINANCE" : "..."}
 ...
 {schwabStatus?.data_source_stats && (schwabStatus.data_source_stats.schwab > 0 || schwabStatus.data_source_stats.yfinance > 0) && (
 <span style={{ fontSize: "10px", color: DIM }}>
 S:{schwabStatus.data_source_stats.schwab} Y:{schwabStatus.data_source_stats.yfinance} F:{schwabStatus.data_source_stats.failed}
 </span>
 )}
 ...
 <div key={i} style={{ display: "flex", gap: "8px", padding: "2px 0", fontSize: "11px" }}>
 <span style={{
 color: a.level === "critical" ? "#0a0a0a" : "#0a0a0a",
 ...
 minWidth: "140px", fontSize: "10px",
 }}>{a.type}</span>
 <span style={{ color: WHITE }}>{a.summary}</span>
 </div>
 ))}
 {alerts.count > 10 && (
 ...
 { label: "Alerts", path: "/api/alerts", desc: "Major signal alerts" },
 ...
 <span style={{ color: YELLOW }}>Schwab not authenticated. </span>
 <span style={{ color: DIM }}>
 To connect: click "Schwab Auth" above, open the URL in your browser, log in,
 then POST the callback code to <span style={{ color: WHITE }}>/api/schwab/callback?code=YOUR_CODE</span>
 </span>
 </div>
 )}
 ...
 | {schwabStatus?.authenticated ? "Schwab" : "yfinance"} data
