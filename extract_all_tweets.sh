...
# Extract all 70 Raul Trades tweets and save to markdown file
...
echo "Starting extraction of ${#urls[@]} tweets..."
echo "Processing will take approximately $((${#urls[@]} * 2)) seconds with 2-second delays"

count=0
for url in "${urls[@]}"; do
    count=$((count + 1))
    echo "[$count/${#urls[@]}] Processing: $url"
    
    # Sleep 2 seconds between requests (except for the first one)
    if [ $count -gt 1 ]; then
        sleep 2
    fi
done
...
echo "Total tweets processed: $count"
